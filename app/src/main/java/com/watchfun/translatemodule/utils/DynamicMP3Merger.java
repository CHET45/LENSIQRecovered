package com.watchfun.translatemodule.utils;
...
public class DynamicMP3Merger {
    ...
    private byte[] concatenate(List<byte[]> list) {
        byte[] bArr = new byte[list.stream().mapToInt(new ToIntFunction() { // from class: vu4
            @Override // java.util.function.ToIntFunction
            public final int applyAsInt(Object obj) {
                return DynamicMP3Merger.lambda$concatenate$0((byte[]) obj);
            }
        }).sum()];
        int length = 0;
        for (byte[] bArr2 : list) {
