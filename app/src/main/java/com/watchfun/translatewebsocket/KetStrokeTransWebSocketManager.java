...
import com.watchfun.translatewebsocket.KetStrokeTransWebSocketManager;
...
public class KetStrokeTransWebSocketManager extends BaseNewWebSocketManager {

    @igg({"StaticFieldLeak"})
    private static volatile KetStrokeTransWebSocketManager mTranslateSocketManager;
    private boolean isSendFileToPlay;
    private OnTtsMp3Listener onTtsMp3Listener;
    private long startTime;
    private final String TAG = "KetStrokeTransWebSocketManager";
    ...
    public KetStrokeTransWebSocketManager() {
        AudioNewRecorder.getInstance().createDefaultAudio();
    }

    public static KetStrokeTransWebSocketManager getInstance() {
        if (mTranslateSocketManager == null) {
            synchronized (KetStrokeTransWebSocketManager.class) {
                try {
                    if (mTranslateSocketManager == null) {
                        mTranslateSocketManager = new KetStrokeTransWebSocketManager();
                    }
                } finally {
                }
            }
    ...
    public /* synthetic */ void lambda$onAudioEnd$0(String str, int i) {
        ...
        C2473f.m4173i("KetStrokeTransWebSocketManager", str + ":::mp3地址:::" + str2);
        try {
    ...
    public /* synthetic */ void lambda$release$2() {
        xt5.deleteFilesInDirWithFilter(new File(getOutputPath()), new FileFilter() { // from class: yp8
            @Override // java.io.FileFilter
            public final boolean accept(File file) {
                return KetStrokeTransWebSocketManager.lambda$release$1(file);
            }
        });
    }
    ...
    public void init() {
        ...
        send(sc7.toJson(new WebsocketNewConfigBean("init", this.language, this.targetLanguage, 12, this.outputRate, this.speakerId, this.asrProvider, this.translationProvider, this.ttsProvider, System.currentTimeMillis())));
    }
    ...
    public void onAudioEnd(String str, int i, String str2, final int i2) {
        ...
        if (str2.contains(st5.f82815c) && !this.isAsring) {
            ...
            C2478k.getSinglePool().execute(new Runnable() { // from class: xp8
                @Override // java.lang.Runnable
                public final void run() {
                    this.f98861a.lambda$onAudioEnd$0(str3, i2);
                }
            });
            ...
            C2473f.m4168e("KetStrokeTransWebSocketManager", "onAudioEnd最后回调音频数据:" + this.ttsFilepath);
        ...
        if (str3.contains(st5.f82815c)) {
            ...
            if (this.asrDataListener != null && this.curStartTimeId.equals(str)) {
                ...
                if (byteArray.length > 0 && !this.isAsring) {
                    Log.e("KetStrokeTransWebSocketManager", "发送pcm数据:" + byteArray.length);
            ...
            if (this.asrDataListener != null && this.curStartTimeId.equals(str) && xt5.isFileExists(str5) && !this.isSendFileToPlay && !this.isAsring && bArrMerge.length > 5000) {
                C2473f.m4173i("KetStrokeTransWebSocketManager", "最后播放音频:" + str5);
    ...
    public void release() {
        ...
        if (!TextUtils.isEmpty(getOutputPath())) {
            C2478k.getCachedPool().execute(new Runnable() { // from class: bq8
                @Override // java.lang.Runnable
                public final void run() {
                    this.f14488a.lambda$release$2();
                }
            });
        }
        resetProvider();
    ...
    public void startWssRecord(String str) {
        if (TextUtils.isEmpty(str)) {
            AudioNewRecorder.getInstance().startRecord(new AudioNewRecorder.RecordStreamListener() { // from class: aq8
                ...
                public final void recordOfByte(byte[] bArr, int i, int i2) {
                    this.f11584a.lambda$startWssRecord$4(bArr, i, i2);
                }
            });
        } else {
            AudioNewRecorder.getInstance().startRecord(str, new AudioNewRecorder.RecordStreamListener() { // from class: zp8
                @Override // com.watchfun.translatewebsocket.AudioNewRecorder.RecordStreamListener
                public final void recordOfByte(byte[] bArr, int i, int i2) {
                    this.f105706a.lambda$startWssRecord$3(bArr, i, i2);
                }
            });
        }
    }
