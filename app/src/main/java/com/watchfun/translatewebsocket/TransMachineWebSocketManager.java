...
public class TransMachineWebSocketManager extends BaseNewWebSocketManager {

    @igg({"StaticFieldLeak"})
    private static volatile TransMachineWebSocketManager mTranslateSocketManager;
    ...
    public TransMachineWebSocketManager() {
        AudioNewRecorder.getInstance().createDefaultAudio();
    }

    public static TransMachineWebSocketManager getInstance() {
        if (mTranslateSocketManager == null) {
            synchronized (TransMachineWebSocketManager.class) {
                try {
                    if (mTranslateSocketManager == null) {
                        mTranslateSocketManager = new TransMachineWebSocketManager();
                    }
                } finally {
                }
            }
    ...
    public /* synthetic */ void lambda$release$2() {
        xt5.deleteFilesInDirWithFilter(new File(getOutputPath()), new FileFilter() { // from class: cch
            @Override // java.io.FileFilter
            public final boolean accept(File file) {
                return TransMachineWebSocketManager.lambda$release$1(file);
            }
        });
    }
    ...
    public void init() {
        ...
        try {
            send(sc7.toJson(new WebsocketNewConfigBean("init", this.language, this.targetLanguage, 6, this.outputRate, this.speakerId, this.asrProvider, this.translationProvider, this.ttsProvider, System.currentTimeMillis())));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public void onAudioEnd(String str, int i, String str2, final int i2) {
        ...
        if (str2.contains(st5.f82815c) && !this.isAsring) {
            ...
            C2478k.getSinglePool().execute(new Runnable() { // from class: ech
                @Override // java.lang.Runnable
                public final void run() {
                    this.f32654a.lambda$onAudioEnd$0(str3, i2);
                }
            });
    ...
    public void release() {
        ...
        if (!TextUtils.isEmpty(getOutputPath())) {
            C2478k.getCachedPool().execute(new Runnable() { // from class: dch
                @Override // java.lang.Runnable
                public final void run() {
                    this.f29361a.lambda$release$2();
                }
            });
        }
        resetProvider();
    ...
    public void startWssRecord(String str) {
        if (TextUtils.isEmpty(str)) {
            AudioNewRecorder.getInstance().startRecord(new AudioNewRecorder.RecordStreamListener() { // from class: gch
                ...
                public final void recordOfByte(byte[] bArr, int i, int i2) {
                    this.f39365a.lambda$startWssRecord$4(bArr, i, i2);
                }
            });
        } else {
            AudioNewRecorder.getInstance().startRecord(str, new AudioNewRecorder.RecordStreamListener() { // from class: fch
                @Override // com.watchfun.translatewebsocket.AudioNewRecorder.RecordStreamListener
                public final void recordOfByte(byte[] bArr, int i, int i2) {
                    this.f36148a.lambda$startWssRecord$3(bArr, i, i2);
                }
            });
        }
    }
    ...
    public void stop(boolean z, String str) {
        ...
        if (this.asrDataListener != null) {
            ...
            sb3.append("stop------------------isHideViewCallback-----------------");
            sb3.append(z);
            if (z) {
