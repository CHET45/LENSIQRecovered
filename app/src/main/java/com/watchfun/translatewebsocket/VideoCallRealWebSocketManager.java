...
public class VideoCallRealWebSocketManager extends BaseNewWebSocketManager {

    @igg({"StaticFieldLeak"})
    private static volatile VideoCallRealWebSocketManager mTranslateSocketManager;
    private boolean isSendFileToPlay;
    private OnTtsMp3Listener onTtsMp3Listener;
    private long startTime;
    private final String TAG = "VideoCallWebSocketManager";
    ...
    public VideoCallRealWebSocketManager() {
        AudioNewRecorder.getInstance().createDefaultAudio();
    }

    public static VideoCallRealWebSocketManager getInstance() {
        if (mTranslateSocketManager == null) {
            synchronized (VideoCallRealWebSocketManager.class) {
                try {
                    if (mTranslateSocketManager == null) {
                        mTranslateSocketManager = new VideoCallRealWebSocketManager();
                    }
                } finally {
                }
            }
    ...
    public /* synthetic */ void lambda$onAudioEnd$0(String str, int i) {
        ...
        C2473f.m4173i("VideoCallWebSocketManager", str + ":::mp3地址:::" + str2);
        try {
    ...
    public /* synthetic */ void lambda$release$2() {
        xt5.deleteFilesInDirWithFilter(new File(getOutputPath()), new FileFilter() { // from class: t5i
            @Override // java.io.FileFilter
            public final boolean accept(File file) {
                return VideoCallRealWebSocketManager.lambda$release$1(file);
            }
        });
    }
    ...
    public void init() {
        ...
        try {
            send(sc7.toJson(new WebsocketNewConfigBean("init", this.language, this.targetLanguage, 8, this.outputRate, this.speakerId, this.asrProvider, this.translationProvider, this.ttsProvider, System.currentTimeMillis())));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public void onAudioEnd(String str, int i, String str2, final int i2) {
        ...
        if (str2.contains(st5.f82815c) && !this.isAsring) {
            ...
            C2478k.getSinglePool().execute(new Runnable() { // from class: s5i
                @Override // java.lang.Runnable
                public final void run() {
                    this.f80734a.lambda$onAudioEnd$0(str3, i2);
                }
            });
            ...
            C2473f.m4168e("VideoCallWebSocketManager", "onAudioEnd最后回调音频数据:" + this.ttsFilepath);
        ...
        if (str3.contains(st5.f82815c)) {
            ...
            if (this.asrDataListener != null && this.curStartTimeId.equals(str)) {
                ...
                if (byteArray.length > 0 && !this.isAsring) {
                    Log.e("VideoCallWebSocketManager", "发送pcm数据:" + byteArray.length);
            ...
            if (this.asrDataListener != null && this.curStartTimeId.equals(str) && xt5.isFileExists(str5) && !this.isSendFileToPlay && !this.isAsring && bArrMerge.length > 5000) {
                C2473f.m4173i("VideoCallWebSocketManager", "最后播放音频:" + str5);
    ...
    public void release() {
        ...
        if (!TextUtils.isEmpty(getOutputPath())) {
            C2478k.getCachedPool().execute(new Runnable() { // from class: p5i
                @Override // java.lang.Runnable
                public final void run() {
                    this.f69761a.lambda$release$2();
                }
            });
        }
        resetProvider();
    ...
    public void startWssRecord(String str) {
        if (TextUtils.isEmpty(str)) {
            AudioNewRecorder.getInstance().startRecord(new AudioNewRecorder.RecordStreamListener() { // from class: r5i
                ...
                public final void recordOfByte(byte[] bArr, int i, int i2) {
                    this.f77102a.lambda$startWssRecord$4(bArr, i, i2);
                }
            });
        } else {
            AudioNewRecorder.getInstance().startRecord(str, new AudioNewRecorder.RecordStreamListener() { // from class: q5i
                @Override // com.watchfun.translatewebsocket.AudioNewRecorder.RecordStreamListener
                public final void recordOfByte(byte[] bArr, int i, int i2) {
                    this.f73295a.lambda$startWssRecord$3(bArr, i, i2);
                }
            });
        }
    }
    ...
    public void stop(boolean z, String str) {
        ...
        sb2.append("stop------------------isAsring-----------22------");
        sb2.append(this.isAsring);
