...
import com.watchfun.translatewebsocket.TransChatFreeWebSocketManager;
...
public class TransChatFreeWebSocketManager extends BaseNewWebSocketManager {

    @igg({"StaticFieldLeak"})
    private static volatile TransChatFreeWebSocketManager mTranslateSocketManager;
    private boolean isSendFileToPlay;
    private OnTtsMp3Listener onTtsMp3Listener;
    private long startTime;
    private final String TAG = "TransChatFreeWebSocketManager";
    ...
    public TransChatFreeWebSocketManager() {
        AudioNewRecorder.getInstance().createDefaultAudio();
    }

    public static TransChatFreeWebSocketManager getInstance() {
        if (mTranslateSocketManager == null) {
            synchronized (TransChatFreeWebSocketManager.class) {
                try {
                    if (mTranslateSocketManager == null) {
                        mTranslateSocketManager = new TransChatFreeWebSocketManager();
                    }
                } finally {
                }
            }
    ...
    public /* synthetic */ void lambda$onAudioEnd$0(String str, int i) {
        ...
        C2473f.m4173i("TransChatFreeWebSocketManager", str + ":::mp3地址:::" + str2);
        try {
    ...
    public /* synthetic */ void lambda$release$2() {
        xt5.deleteFilesInDirWithFilter(new File(getOutputPath()), new FileFilter() { // from class: y8h
            @Override // java.io.FileFilter
            public final boolean accept(File file) {
                return TransChatFreeWebSocketManager.lambda$release$1(file);
            }
        });
    }
    ...
    public void init() {
        ...
        send(sc7.toJson(new WebsocketNewConfigBean("init", this.language, this.targetLanguage, 2, this.outputRate, this.speakerId, this.asrProvider, this.translationProvider, this.ttsProvider, System.currentTimeMillis())));
    }
    ...
    public void onAudioEnd(String str, int i, String str2, final int i2) {
        ...
        if (TextUtils.isEmpty(str)) {
            C2473f.m4173i("TransChatFreeWebSocketManager", "onAudioEnd: startTimeId为空，跳过处理");
            return;
        }
        if (str2.contains(st5.f82815c) && !this.isAsring) {
            final String str3 = getOutputPath() + pj4.f71071b + str + ".pcm";
            C2478k.getSinglePool().execute(new Runnable() { // from class: x8h
                @Override // java.lang.Runnable
                public final void run() {
                    this.f97206a.lambda$onAudioEnd$0(str3, i2);
                }
            });
            ...
            C2473f.m4168e("TransChatFreeWebSocketManager", "onAudioEnd最后回调音频数据:" + this.ttsFilepath);
        ...
        if (str3.contains(st5.f82815c)) {
            ...
            if (this.asrDataListener != null && this.curStartTimeId.equals(str)) {
                ...
                if (byteArray.length > 0 && !this.isAsring) {
                    Log.e("TransChatFreeWebSocketManager", "发送pcm数据:" + byteArray.length);
            ...
            if (this.asrDataListener != null && this.curStartTimeId.equals(str) && xt5.isFileExists(str5) && !this.isSendFileToPlay && !this.isAsring && bArrMerge.length > 5000) {
                C2473f.m4173i("TransChatFreeWebSocketManager", "最后播放音频:" + str5);
    ...
    public void release() {
        ...
        if (!TextUtils.isEmpty(getOutputPath())) {
            C2478k.getCachedPool().execute(new Runnable() { // from class: z8h
                @Override // java.lang.Runnable
                public final void run() {
                    this.f104384a.lambda$release$2();
                }
            });
        }
        resetProvider();
    ...
    public void startWssRecord(String str) {
        if (TextUtils.isEmpty(str)) {
            AudioNewRecorder.getInstance().startRecord(new AudioNewRecorder.RecordStreamListener() { // from class: w8h
                ...
                public final void recordOfByte(byte[] bArr, int i, int i2) {
                    this.f93621a.lambda$startWssRecord$4(bArr, i, i2);
                }
            });
        } else {
            AudioNewRecorder.getInstance().startRecord(str, new AudioNewRecorder.RecordStreamListener() { // from class: v8h
                @Override // com.watchfun.translatewebsocket.AudioNewRecorder.RecordStreamListener
                public final void recordOfByte(byte[] bArr, int i, int i2) {
                    this.f90347a.lambda$startWssRecord$3(bArr, i, i2);
                }
            });
        }
    }
