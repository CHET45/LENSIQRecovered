...
import com.watchfun.translatewebsocket.TransTwoEarWebSocketManager;
...
public class TransTwoEarWebSocketManager extends BaseNewWebSocketManager {

    @igg({"StaticFieldLeak"})
    private static volatile TransTwoEarWebSocketManager mTranslateSocketManager;
    private boolean isSendFileToPlay;
    private OnTtsMp3Listener onTtsMp3Listener;
    private long startTime;
    private final String TAG = "TransTwoEarWebSocketManager";
    ...
    public TransTwoEarWebSocketManager() {
        AudioNewRecorder.getInstance().createDefaultAudio();
    }

    public static TransTwoEarWebSocketManager getInstance() {
        if (mTranslateSocketManager == null) {
            synchronized (TransTwoEarWebSocketManager.class) {
                try {
                    if (mTranslateSocketManager == null) {
                        mTranslateSocketManager = new TransTwoEarWebSocketManager();
                    }
                } finally {
                }
            }
    ...
    public /* synthetic */ void lambda$onAudioEnd$0(String str, int i) {
        ...
        C2473f.m4173i("TransTwoEarWebSocketManager", str + ":::mp3地址:::" + str2);
        try {
    ...
    public /* synthetic */ void lambda$release$2() {
        xt5.deleteFilesInDirWithFilter(new File(getOutputPath()), new FileFilter() { // from class: sch
            @Override // java.io.FileFilter
            public final boolean accept(File file) {
                return TransTwoEarWebSocketManager.lambda$release$1(file);
            }
        });
    }
    ...
    public void init() {
        ...
        send(sc7.toJson(new WebsocketNewConfigBean("init", this.language, this.targetLanguage, 5, this.outputRate, this.speakerId, this.asrProvider, this.translationProvider, this.ttsProvider, System.currentTimeMillis())));
    }
    ...
    public void onAudioEnd(String str, int i, String str2, final int i2) {
        ...
        if (str2.contains(st5.f82815c) && !this.isAsring) {
            ...
            C2478k.getSinglePool().execute(new Runnable() { // from class: rch
                @Override // java.lang.Runnable
                public final void run() {
                    this.f77839a.lambda$onAudioEnd$0(str3, i2);
                }
            });
            ...
            C2473f.m4168e("TransTwoEarWebSocketManager", "onAudioEnd最后回调音频数据:" + this.ttsFilepath);
        ...
        if (str3.contains(st5.f82815c)) {
            ...
            if (this.asrDataListener != null && this.curStartTimeId.equals(str)) {
                ...
                if (byteArray.length > 0 && !this.isAsring) {
                    Log.e("TransTwoEarWebSocketManager", "发送pcm数据:" + byteArray.length);
            ...
            if (this.asrDataListener != null && this.curStartTimeId.equals(str) && xt5.isFileExists(str5) && !this.isSendFileToPlay && !this.isAsring && bArrMerge.length > 5000) {
                C2473f.m4173i("TransTwoEarWebSocketManager", "最后播放音频:" + str5);
    ...
    public void release() {
        ...
        if (!TextUtils.isEmpty(getOutputPath())) {
            C2478k.getCachedPool().execute(new Runnable() { // from class: vch
                @Override // java.lang.Runnable
                public final void run() {
                    this.f90699a.lambda$release$2();
                }
            });
        }
        resetProvider();
    ...
    public void startWssRecord(String str) {
        if (TextUtils.isEmpty(str)) {
            AudioNewRecorder.getInstance().startRecord(new AudioNewRecorder.RecordStreamListener() { // from class: uch
                ...
                public final void recordOfByte(byte[] bArr, int i, int i2) {
                    this.f87598a.lambda$startWssRecord$4(bArr, i, i2);
                }
            });
        } else {
            AudioNewRecorder.getInstance().startRecord(str, new AudioNewRecorder.RecordStreamListener() { // from class: tch
                @Override // com.watchfun.translatewebsocket.AudioNewRecorder.RecordStreamListener
                public final void recordOfByte(byte[] bArr, int i, int i2) {
                    this.f84286a.lambda$startWssRecord$3(bArr, i, i2);
                }
            });
        }
    }
