...
public class jdb {
    ...
    private void validateNoTypeParameterOnType(Type type, List<Throwable> list) {
        if (type instanceof TypeVariable) {
            list.add(new Exception("Method " + this.f50356a.getName() + "() contains unresolved type variable " + type));
            return;
        }
    ...
    /* JADX INFO: renamed from: a */
    public void m14064a(List<Throwable> list) {
        for (Type type : this.f50356a.getGenericParameterTypes()) {
            validateNoTypeParameterOnType(type, list);
        }
    }
}
