...
public class qya extends db3 {

    /* JADX INFO: renamed from: i */
    public static final String f76303i = "MonotonicCurveFit";

    /* JADX INFO: renamed from: d */
    public double[] f76304d;

    /* JADX INFO: renamed from: e */
    public double[][] f76305e;

    /* JADX INFO: renamed from: f */
    public double[][] f76306f;

    /* JADX INFO: renamed from: g */
    public boolean f76307g = true;

    /* JADX INFO: renamed from: h */
    public double[] f76308h;

    public qya(double[] dArr, double[][] dArr2) {
        int length = dArr.length;
        int length2 = dArr2[0].length;
        this.f76308h = new double[length2];
        int i = length - 1;
    ...
    public static qya buildWave(String str) {
    ...
    public void getPos(double d, double[] dArr) {
        double[] dArr2 = this.f76304d;
        int length = dArr2.length;
        int i = 0;
        int length2 = this.f76305e[0].length;
        if (this.f76307g) {
            double d2 = dArr2[0];
            if (d <= d2) {
                getSlope(d2, this.f76308h);
                for (int i2 = 0; i2 < length2; i2++) {
                    dArr[i2] = this.f76305e[0][i2] + ((d - this.f76304d[0]) * this.f76308h[i2]);
                }
                return;
            }
            int i3 = length - 1;
            double d3 = dArr2[i3];
            if (d >= d3) {
                getSlope(d3, this.f76308h);
                while (i < length2) {
                    dArr[i] = this.f76305e[i3][i] + ((d - this.f76304d[i3]) * this.f76308h[i]);
                    i++;
                }
                return;
            }
        ...
        int i6 = 0;
        while (i6 < length - 1) {
            if (d == this.f76304d[i6]) {
                for (int i7 = 0; i7 < length2; i7++) {
                    dArr[i7] = this.f76305e[i6][i7];
                }
            }
            double[] dArr3 = this.f76304d;
            int i8 = i6 + 1;
            double d4 = dArr3[i8];
            if (d < d4) {
                double d5 = dArr3[i6];
                ...
                double d7 = (d - d5) / d6;
                while (i < length2) {
                    double[][] dArr4 = this.f76305e;
                    double d8 = dArr4[i6][i];
                    double d9 = dArr4[i8][i];
                    double[][] dArr5 = this.f76306f;
    ...
    private static qya buildWave(double[] dArr) {
        ...
        return new qya(dArr3, dArr2);
    }

    @Override // p000.db3
    public double getSlope(double d, int i) {
        double[] dArr = this.f76304d;
        int length = dArr.length;
        int i2 = 0;
        double d2 = dArr[0];
        if (d >= d2) {
        ...
        while (i2 < length - 1) {
            ...
            if (d2 <= d3) {
                double d4 = dArr2[i2];
                double d5 = d3 - d4;
                double[][] dArr3 = this.f76305e;
                double d6 = dArr3[i2][i];
                double d7 = dArr3[i3][i];
                double[][] dArr4 = this.f76306f;
    ...
    public void getPos(double d, float[] fArr) {
        ...
        if (this.f76307g) {
            ...
            if (d <= d2) {
                getSlope(d2, this.f76308h);
                for (int i2 = 0; i2 < length2; i2++) {
                    fArr[i2] = (float) (this.f76305e[0][i2] + ((d - this.f76304d[0]) * this.f76308h[i2]));
                }
                return;
            }
            int i3 = length - 1;
            ...
            if (d >= d3) {
                getSlope(d3, this.f76308h);
                while (i < length2) {
                    fArr[i] = (float) (this.f76305e[i3][i] + ((d - this.f76304d[i3]) * this.f76308h[i]));
                    i++;
                }
                return;
            }
            ...
            if (d <= dArr[0]) {
                for (int i4 = 0; i4 < length2; i4++) {
                    fArr[i4] = (float) this.f76305e[0][i4];
                }
                return;
            }
            int i5 = length - 1;
            if (d >= dArr[i5]) {
                while (i < length2) {
                    fArr[i] = (float) this.f76305e[i5][i];
                    i++;
                }
                return;
            }
        ...
        while (i6 < length - 1) {
            if (d == this.f76304d[i6]) {
                for (int i7 = 0; i7 < length2; i7++) {
                    fArr[i7] = (float) this.f76305e[i6][i7];
                }
            }
            double[] dArr2 = this.f76304d;
            int i8 = i6 + 1;
    ...
    public double getPos(double d, int i) {
        ...
        if (d <= dArr[0]) {
            return this.f76305e[0][i];
        }
        int i4 = length - 1;
        if (d >= dArr[i4]) {
            return this.f76305e[i4][i];
        }
        while (i2 < length - 1) {
            double[] dArr2 = this.f76304d;
            double d6 = dArr2[i2];
            if (d == d6) {
                return this.f76305e[i2][i];
            }
            int i5 = i2 + 1;
            double d7 = dArr2[i5];
            if (d < d7) {
