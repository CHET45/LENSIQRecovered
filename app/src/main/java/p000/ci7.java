...
public class ci7 {
    ...
    public static double[] m3968b(double d, double d2) {
        ...
        return m3979m(dArr, dArr2);
    }

    /* JADX INFO: renamed from: c */
    public static double[][] m3969c(double d, double d2) {
        ...
        boolean z2 = true;
        for (int i = 0; i < 12; i++) {
            double[] dArrM3980n = m3980n(d, i);
            if (dArrM3980n[0] >= 0.0d) {
                double dM3974h = m3974h(dArrM3980n);
                if (!z) {
                    z = true;
                    dArr = dArrM3980n;
                    dArr2 = dArr;
                    d3 = dM3974h;
                    d4 = d3;
                } else if (z2 || m3967a(d3, dM3974h, d4)) {
                    if (m3967a(d3, d2, dM3974h)) {
                        z2 = false;
                        dArr2 = dArrM3980n;
                        d4 = dM3974h;
                    } else {
    ...
    /* JADX INFO: renamed from: d */
    public static double m3970d(double d) {
        double dPow = Math.pow(Math.abs(d), 0.42d);
        return ((((double) s3a.signum(d)) * 400.0d) * dPow) / (dPow + 27.13d);
    }

    /* JADX INFO: renamed from: e */
    public static int m3971e(double d) {
        return (int) Math.ceil(d - 0.5d);
    }

    /* JADX INFO: renamed from: f */
    public static int m3972f(double d) {
        return (int) Math.floor(d - 0.5d);
    }

    /* JADX INFO: renamed from: g */
    public static int m3973g(double d, double d2, double d3) {
        double dSqrt = Math.sqrt(d3) * 11.0d;
        sci sciVar = sci.f81232k;
        double d4 = 1.0d;
        double dPow = 1.0d / Math.pow(1.64d - Math.pow(0.29d, sciVar.getN()), 0.73d);
        double dCos = (Math.cos(d + 2.0d) + 3.8d) * 0.25d * 3846.153846153846d * sciVar.m21824c() * sciVar.m21825d();
        ...
        while (i < 5) {
            ...
            double aw = (sciVar.getAw() * Math.pow(d5, (d4 / sciVar.m21822a()) / sciVar.m21826e())) / sciVar.getNbb();
            double d7 = (((0.305d + aw) * 23.0d) * dPow2) / (((23.0d * d6) + ((dPow2 * 11.0d) * dCos2)) + ((dPow2 * 108.0d) * dSin));
            double d8 = d7 * dCos2;
            double d9 = d7 * dSin;
            double d10 = aw * 460.0d;
            double[] dArrMatrixMultiply = s3a.matrixMultiply(new double[]{m3976j((((451.0d * d8) + d10) + (288.0d * d9)) / 1403.0d), m3976j(((d10 - (891.0d * d8)) - (261.0d * d9)) / 1403.0d), m3976j(((d10 - (d8 * 220.0d)) - (d9 * 6300.0d)) / 1403.0d)}, f16624b);
            ...
            if (dArrMatrixMultiply[0] <= 100.01d && dArrMatrixMultiply[1] <= 100.01d && dArrMatrixMultiply[2] <= 100.01d) {
                return xa2.argbFromLinrgb(dArrMatrixMultiply);
            }
        }
        return 0;
    }
    ...
    public static double m3974h(double[] dArr) {
        double[] dArrMatrixMultiply = s3a.matrixMultiply(dArr, f16623a);
        double dM3970d = m3970d(dArrMatrixMultiply[0]);
        double dM3970d2 = m3970d(dArrMatrixMultiply[1]);
        double dM3970d3 = m3970d(dArrMatrixMultiply[2]);
        return Math.atan2(((dM3970d + dM3970d2) - (dM3970d3 * 2.0d)) / 9.0d, (((dM3970d * 11.0d) + ((-12.0d) * dM3970d2)) + dM3970d3) / 11.0d);
    }

    /* JADX INFO: renamed from: i */
    public static double m3975i(double d, double d2, double d3) {
        return (d2 - d) / (d3 - d);
    }

    /* JADX INFO: renamed from: j */
    public static double m3976j(double d) {
        double dAbs = Math.abs(d);
        return ((double) s3a.signum(d)) * Math.pow(Math.max(0.0d, (27.13d * dAbs) / (400.0d - dAbs)), 2.380952380952381d);
    }

    /* JADX INFO: renamed from: k */
    public static boolean m3977k(double d) {
        return 0.0d <= d && d <= 100.0d;
    }

    /* JADX INFO: renamed from: l */
    public static double[] m3978l(double[] dArr, double d, double[] dArr2) {
        double d2 = dArr[0];
    ...
    public static double[] m3979m(double[] dArr, double[] dArr2) {
        return new double[]{(dArr[0] + dArr2[0]) / 2.0d, (dArr[1] + dArr2[1]) / 2.0d, (dArr[2] + dArr2[2]) / 2.0d};
    }

    /* JADX INFO: renamed from: n */
    public static double[] m3980n(double d, int i) {
        double[] dArr = f16625c;
        double d2 = dArr[0];
        double d3 = dArr[1];
        double d4 = dArr[2];
        ...
        if (i < 4) {
            ...
            return m3977k(d7) ? new double[]{d7, d5, d6} : new double[]{-1.0d, -1.0d, -1.0d};
        }
        if (i < 8) {
            double d8 = ((d - (d2 * d6)) - (d4 * d5)) / d3;
            return m3977k(d8) ? new double[]{d6, d8, d5} : new double[]{-1.0d, -1.0d, -1.0d};
        }
        double d9 = ((d - (d2 * d5)) - (d3 * d6)) / d4;
        return m3977k(d9) ? new double[]{d5, d6, d9} : new double[]{-1.0d, -1.0d, -1.0d};
    }

    /* JADX INFO: renamed from: o */
    public static double m3981o(double d) {
        return (d + 25.132741228718345d) % 6.283185307179586d;
    }

    /* JADX INFO: renamed from: p */
    public static double[] m3982p(double[] dArr, double d, double[] dArr2, int i) {
        return m3978l(dArr, m3975i(dArr[i], d, dArr2[i]), dArr2);
    }

    /* JADX INFO: renamed from: q */
    public static double m3983q(double d) {
        double d2 = d / 100.0d;
        return (d2 <= 0.0031308d ? d2 * 12.92d : (Math.pow(d2, 0.4166666666666667d) * 1.055d) - 0.055d) * 255.0d;
    }

    public static xo1 solveToCam(double d, double d2, double d3) {
        return xo1.fromInt(solveToInt(d, d2, d3));
    }

    public static int solveToInt(double d, double d2, double d3) {
        if (d2 < 1.0E-4d || d3 < 1.0E-4d || d3 > 99.9999d) {
            return xa2.argbFromLstar(d3);
        }
        double dSanitizeDegreesDouble = (s3a.sanitizeDegreesDouble(d) / 180.0d) * 3.141592653589793d;
        double dYFromLstar = xa2.yFromLstar(d3);
        int iM3973g = m3973g(dSanitizeDegreesDouble, d2, dYFromLstar);
        return iM3973g != 0 ? iM3973g : xa2.argbFromLinrgb(m3968b(dYFromLstar, dSanitizeDegreesDouble));
    }
}
